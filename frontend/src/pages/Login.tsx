...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Bate na porta do nosso guarda-costas no NestJS
      const response = await fetch('/auth/login', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        // 2. Entrega o e-mail e a senha que o utilizador digitou
        body: JSON.stringify({ email, senha }),
      });

      // Se o NestJS devolver um erro (ex: 401 Unauthorized), caímos aqui
      if (!response.ok) {
        // Lemos como texto primeiro, para não dar erro se vier vazio!
        const text = await response.text(); 
        let errorMessage = `Erro do servidor (Status: ${response.status})`;
        try {
          const errorData = JSON.parse(text); // Tentamos converter para JSON
          errorMessage = errorData.message || errorMessage;
        } catch (e) {
          // Se não for JSON (ex: servidor offline), mostramos o erro padrão acima
        }
        throw new Error(errorMessage);
      }

      // 3. Se deu tudo certo, pegamos o Token JWT!
      const data = await response.json();

      // 4. Guardamos o "crachá" no navegador para o utilizador não ter de fazer login toda a hora
      localStorage.setItem('token_farmacia', data.token);
      localStorage.setItem('usuario_farmacia', JSON.stringify(data.usuario));
